...
def get_bert_biencoder_components(cfg, inference_only: bool = False, **kwargs):
    ...
    #kobert일 경우 tensorizer 
    if cfg.encoder['pretrained_model_cfg'] == 'monologg/kobert':
      logger.info("###get kobert tensorizer")
      tensorizer = get_kobert_tensorizer(cfg)
    else: tensorizer = get_bert_tensorizer(cfg)
...
#kobert tensorizer
def get_kobert_tensorizer(cfg):
    sequence_length = cfg.encoder.sequence_length
    pretrained_model_cfg = cfg.encoder.pretrained_model_cfg
    tokenizer = get_kobert_tokenizer(pretrained_model_cfg, do_lower_case=cfg.do_lower_case)
    if cfg.special_tokens:
        _add_special_tokens(tokenizer, cfg.special_tokens)

    return KoBertTensorizer(tokenizer, sequence_length)
...
def get_kobert_tokenizer(pretrained_cfg_name: str, do_lower_case: bool = True):
...
class HFBertEncoder(BertModel):
    ...
    def init_encoder(
        ...
        #코버트일때 config 
        if cfg_name == 'monologg/kobert': 
            cfg = BertConfig(max_position_embeddings = 512, \
                            vocab_size = 8002, \
...
#Kobert tensorizer
class KoBertTensorizer(BertTensorizer):
    def __init__(self, tokenizer, max_length: int, pad_to_max: bool = True):
        super(KoBertTensorizer, self).__init__(tokenizer, max_length, pad_to_max=pad_to_max)
